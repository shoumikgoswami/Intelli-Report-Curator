...
from pdf_generator import gen_image_64, write_to_html_file, gen_pdf
...
def plot_builder(mode, plot_type, data):
	if plot_type == 'distribution plot':
		...
		plot = distributionplot(data)
		gen_output(mode,'Do you want to save the plot?')
		text = get_text(mode)
		if text.lower() == 'yes':
			cmd_add_to_pdf('',plot)
...
def cmd_data_exploration(mode,data):
	...
	if explore_type == 'head':
		...
		df = data.head(row_num)
		print(df)
		gen_output(mode,'Do you want to add to report?')
		text = get_text(mode)
		if text.lower() == 'yes':
			cmd_add_to_pdf(df,'')
		...
		if data.isnull().values.any() == True:
			...
			return 
# for sales report			
	elif explore_type == 'max_sales_units':
		print(data.columns.values)
		col_names = list(data.columns.values)
		gen_output(mode, 'Please select the ID column from the list of column names')
		col = get_text(mode)
		if col in col_names:
			X = data[col]
			gen_output(mode,"ID column set")
		else:
			gen_output(mode,"Column not found")
			cmd_data_exploration(mode,data)
		df = data.groupby([X.name]).sum().sort_values('units', ascending=False).reset_index()
		print(df)
		gen_output(mode,'Do you want to add to report?')
		text = get_text(mode)
		if text.lower() == 'yes':
		...
		if text.lower() == 'yes':
			...
			return
		return 
	elif explore_type == 'max_sales_price':
		print(data.columns.values)
		col_names = list(data.columns.values)
		gen_output(mode, 'Please select the ID column from the list of column names')
		col = get_text(mode)
		if col in col_names:
			...
			gen_output(mode,"Column not found")
			cmd_data_exploration(mode,data)
		df = data.groupby([X.name]).sum().sort_values('total price', ascending=False).reset_index()
		print(df)
		gen_output(mode,'Do you want to add to report?')
		text = get_text(mode)
		if text.lower() == 'yes':
		...
		if text.lower() == 'yes':
			...
			return
		return 
	elif explore_type == 'sales_data':
		print('List of sales personnel: ', data['rep'].unique)
		gen_output(mode, 'Please enter the name of the sales person from the list')
		text = get_text(mode)
		data['rep'] = data['rep'].str.lower()
		df = data[data['rep']==text]
		print(df)
		gen_output(mode,'Do you want to add to report?')
		text = get_text(mode)
		if text.lower() == 'yes':
...
def cmd_download_pdf(df_list='',img_list=''):
	gen_pdf(df_list,img_list)
	gen_output(mode,'Done!')
	
# ********************** Initiate Anna **********************

print("************* Intelli Report Curator *************")
engine.say("Initiating the curator engine")
engine.runAndWait()
time.sleep(2)
gen_output('VOICE',"Hello! I am your personal data analysis assistant. I can help you analyze your data and generate personalized reports.")
...
try:
	while True:
		...
		if cmd == '1U':
			...
			cmd_add_to_pdf(data,plot)
		elif cmd == '9PC':
			cmd_download_pdf(df_list,img_list)	
		elif cmd == 'bye':
